...
function doExport() {
  ...
  try {
    ...
    if (msg) {
      msg.textContent = 'Backup saved to local storage on this device.';
      msg.style.color = 'var(--mint)';
    }
  } catch {
    if (msg) {
      msg.textContent = 'Export failed — local storage may be full.';
...
function drawHome() {
  ...
  document.getElementById('metrics').innerHTML = [
    ...
    { label:'People', value: store.people.length, color: COLORS[2] },
    { label:'Open Tasks', value: open, color: COLORS[1] },
    { label:'Classes', value: store.classes.length, color: COLORS[3] },
  ].map(m => `
  ...
  document.getElementById('comingUp').innerHTML = bdays.length
    ? bdays.map(p => `
      <div class="row">
        ...
        <span class="row-meta">${parseD(p.bday).toLocaleDateString('en-US',{month:'short',day:'numeric'})}</span>
      </div>`).join('')
  ...
  list.innerHTML = items.length ? items.map(p => `
    <div class="card" data-pid="${p.id}" style="cursor:pointer;">
      ...
      <div class="card-body">
        ...
        ${p.bday?`<div class="card-info">𖣐 ${parseD(p.bday).toLocaleDateString('en-US',{month:'short',day:'numeric'})}</div>`:''}
  ...
  list.querySelectorAll('[data-in],[data-out]').forEach(btn => {
    btn.onclick = () => {
      ...
      form.querySelector('.conf').onclick = () => {
        ...
        if (!acc) return;
        acc.tx = acc.tx || [];
        acc.tx.push({ id: id(), desc: note, amt: isIn ? amt : -amt, date: today() });
...
/* Splash screen — shown while the app boots, then fades out */
const SPLASH_MIN_MS = 5100;
const splashStart = Date.now();
function hideSplash() {
  const el = document.getElementById('nexusSplash');
  if (!el) return;
